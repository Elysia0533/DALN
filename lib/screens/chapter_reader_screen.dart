...
class _ChapterReaderScreenState extends State<ChapterReaderScreen> {
  ...
  Future<void> _loadEpub() async {
    ...
    try {
      ...
      setState(() { _chapters = flat; _isLoading = false; });
    } catch (e) {
  ...
  void _flattenChapters(List<epubx.EpubChapter> list, List<_Chapter> out) {
    for (final ch in list) {
      ...
      if (plain.trim().isNotEmpty) {
        out.add(_Chapter(title: ch.Title ?? 'Chương ${out.length + 1}', html: html, plain: plain));
      }
      if (ch.SubChapters != null) _flattenChapters(ch.SubChapters!, out);
    }
  }
  ...
  void _onScroll() {
    ...
    if (_scrollController.position.userScrollDirection == ScrollDirection.reverse && _showBars) {
      setState(() => _showBars = false);
    } else if (_scrollController.position.userScrollDirection == ScrollDirection.forward && !_showBars) {
  ...
  Future<void> _goToChapter(int index, {bool smooth = true}) async {
    if (index < 0 || index >= _chapters.length || index == _currentIndex) return;
    await _stopTts();
    setState(() => _currentIndex = index);
    ApiService.saveChapterProgress(widget.story.id, index);
    if (smooth) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_isLoading) return Scaffold(backgroundColor: settings.bgColor, body: const Center(child: CircularProgressIndicator()));
    if (_error != null) return Scaffold(body: Center(child: Text('Lỗi: $_error')));
    if (_chapters.isEmpty) return const Scaffold(body: Center(child: Text('Không tìm thấy nội dung')));
    ...
    return Scaffold(
      ...
      drawer: _TocDrawer(
        ...
        onSelect: (i) { Navigator.pop(context); _goToChapter(i); },
      ),
          ...
          children: [
            ...
            NotificationListener<ScrollNotification>(
              child: CustomScrollView(
                ...
                slivers: [
                  SliverToBoxAdapter(child: SizedBox(height: _showBars ? 100 : 60)),
                  // Chapter title
                  SliverToBoxAdapter(
                    child: Padding(
                    ...
                    child: Padding(
                      ...
                      child: _currentIndex < _chapters.length - 1
                          ? FilledButton.icon(
                              ...
                              label: Text('Chương tiếp: ${_chapters[_currentIndex + 1].title}',
                          ...
                          : Center(
                              child: Text('🎉 Đã đọc hết truyện!',
                                  style: TextStyle(color: settings.textColor.withValues(alpha: 0.6), fontSize: 16)),
                            ),
                    ),
                  ),
                ],
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(widget.story.title,
                                style: settings.bodyTextStyle.copyWith(fontSize: 15, fontWeight: FontWeight.w600),
                                maxLines: 1, overflow: TextOverflow.ellipsis),
                          ),
                          IconButton(icon: Icon(Icons.menu_book_outlined, color: settings.textColor), onPressed: _showToc),
                          IconButton(icon: Icon(Icons.text_fields_rounded, color: settings.textColor), onPressed: _showSettings),
                          IconButton(
                            icon: Icon(
                              _isSpeaking && !_isPaused ? Icons.pause_circle_outline : Icons.play_circle_outline,
                        ...
                        children: [
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: [
                                IconButton(
                                  icon: Icon(Icons.skip_previous_rounded, color: settings.textColor.withValues(alpha: _currentIndex > 0 ? 1 : 0.3)),
                                  onPressed: _currentIndex > 0 ? () => _goToChapter(_currentIndex - 1) : null,
                                ),
                                Expanded(
                                  child: Column(
                                    children: [
                                      SliderTheme(
                                        ...
                                        child: Slider(
                                          value: progress,
                                          onChanged: (v) => _goToChapter((v * (_chapters.length - 1)).round()),
                                      ...
                                      Text(
                                        ...
                                        style: TextStyle(fontSize: 12, color: settings.textColor.withValues(alpha: 0.6)),
                                      ),
                                    ],
                                  ),
                                ),
                                IconButton(
                                  icon: Icon(Icons.skip_next_rounded, color: settings.textColor.withValues(alpha: _currentIndex < _chapters.length - 1 ? 1 : 0.3)),
                                  onPressed: _currentIndex < _chapters.length - 1 ? () => _goToChapter(_currentIndex + 1) : null,
                                ),
                              ],
                            ),
                          ),
...
class _TocDrawer extends StatefulWidget {
  ...
  const _TocDrawer({required this.chapters, required this.currentIndex, required this.onSelect});
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    child: Text('${widget.chapters.length} chương', style: TextStyle(fontSize: 12, color: accent, fontWeight: FontWeight.w600)),
                  ),
                ],
              ),
            ),
                ...
                itemBuilder: (_, i) {
                  ...
                  return InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: isCur ? accent.withValues(alpha: 0.15) : Colors.transparent, // Bôi màu đậm hơn chút
                        border: Border(left: BorderSide(color: isCur ? accent : Colors.transparent, width: 4)),
                      ),
                      child: Row(
                        children: [
                          Expanded(
                            child: Text(
                              ...
                              style: TextStyle(fontSize: 15, fontWeight: isCur ? FontWeight.w700 : FontWeight.normal, color: isCur ? accent : null),
                            ),
                          ),
                          if (isCur) Icon(Icons.play_arrow_rounded, color: accent, size: 22),
                        ],
                      ),
                    ),
                  );
        ...
        children: [
          ...
          Row(
            children: [
              ...
              SizedBox(width: 36, child: Text('${settings.fontSize.toInt()}', textAlign: TextAlign.center,
          ...
          Row(
            children: [
              ...
              SizedBox(width: 36, child: Text(settings.lineHeight.toStringAsFixed(1), textAlign: TextAlign.center,
                  style: TextStyle(fontWeight: FontWeight.bold, color: textColor))),
            ],
          ),
